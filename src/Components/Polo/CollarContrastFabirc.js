...
export const CollarContrastFabricUI = () => {
  ...
  return (
    ...
    <div className=" mt-4">
    <article className="text-white text-[15px] font-[AcuminVariable]">Collar Fabric Option</article>
    <div className="flex flex-row  bg-[#15171F] overflow-x-auto space-x-[7.64px]">

        {Fabricimages.map((img, index) => (
          <button key={index} onClick={() => selectFabric(index)}  >
            <img src={img} alt={`Image ${index + 1}`}
            className={`h-[40.91px] w-[36.04px] ${sharedState.CollarFabricIndex === index ? 'border-2 border-yellow-600' : ''}`} />
          </button>
        ))}
      </div>
    </div>
